...
def qjoin_endpoint():
    ...
    return qjoin(request.form['user_id'], request.form['user_name'], queue_name)

def qjoin(user_id, user_name, queue_name):
    ...
    if user_id not in [id for id in queues[queue_name]]:
        ...
        enqueued_users[user_id] = {'joined': datetime.datetime.now(), 'name': user_name, 'queue_name': queue_name}
...
def route_to_action():
    ...
    if action == "qjoin":
        ...
        return qjoin(user_id, user_name, channel_name)
    if action == "qleave":
